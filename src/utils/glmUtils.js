export function fitSigmoid(hues, responses, polarity, tailProbability = 0.2) {
  ...
  for (let iter = 0; iter < 10; iter++) {
    ...
    const grad_a =
      hues.reduce((sum, hue, i) => {
        const z = a * (hue - 60 + b)
        const s = sigmoid(z)
        return sum + (responses[i] - s) * (hue - 60 + b)
      }, 0) -
    ...
    const hess_aa =
      hues.reduce((sum, hue) => {
        const z = a * (hue - 60 + b)
        const s = sigmoid(z)
        return sum - s * (1 - s) * (hue - 60 + b) ** 2
      }, 0) -
      1 / priorA.sd ** 2
    const hess_ab = hues.reduce((sum, hue) => {
      const z = a * (hue - 60 + b)
      const s = sigmoid(z)
      return sum - s * (1 - s) * a * (hue - 60 + b)
    }, 0)
    const hess_bb =
      hues.reduce((sum, hue) => {
        const z = a * (hue - 60 + b)
  ...
  let newProbe = 60 - b + Math.log(percentile / (1 - percentile)) / a
  newProbe = Math.max(0, Math.min(newProbe, 120))
