export function fitSigmoid(hues, responses, polarity, tailProbability = 0.2) {
  ...
  const logLikelihood = (a, b) => {
    return hues.reduce((sum, hue, i) => {
      const z = a * (hue - COLOR_HUE_MIDPOINT + b)
  ...
  for (let iter = 0; iter < 10; iter++) {
    ...
    const grad_a =
      hues.reduce((sum, hue, i) => {
        const z = a * (hue - COLOR_HUE_MIDPOINT + b)
        const s = sigmoid(z)
        return sum + (responses[i] - s) * (hue - COLOR_HUE_MIDPOINT + b)
      }, 0) -
    ...
    const hess_aa =
      hues.reduce((sum, hue) => {
        const z = a * (hue - COLOR_HUE_MIDPOINT + b)
        const s = sigmoid(z)
        return sum - s * (1 - s) * (hue - COLOR_HUE_MIDPOINT + b) ** 2
      }, 0) -
      1 / priorA.sd ** 2
    const hess_ab = hues.reduce((sum, hue) => {
      const z = a * (hue - COLOR_HUE_MIDPOINT + b)
      const s = sigmoid(z)
      return sum - s * (1 - s) * a * (hue - COLOR_HUE_MIDPOINT + b)
    }, 0)
    const hess_bb =
      hues.reduce((sum, hue) => {
        const z = a * (hue - COLOR_HUE_MIDPOINT + b)
  ...
  let newProbe = COLOR_HUE_MIDPOINT - b + Math.log(percentile / (1 - percentile)) / a
  newProbe = Math.max(COLOR_HUE_A, Math.min(newProbe, COLOR_HUE_B))
