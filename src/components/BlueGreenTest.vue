      ...
      <div v-if="rounds < MAX_ROUNDS" class="blue-green-test-content">
        <transition name="fade-up" mode="out-in">
          ...
          <h1 v-else key="main" class="blue-green-test-title">
            <span class="background-white">Blue Red Threshold Test</span>
          </h1>
        </transition>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      currentHue: Math.random() > 0.5 ? 240 : 360,
  ...
  methods: {
    selectColor(color) {
      ...
      const { b, newProbe, polarity } = fitSigmoid(
        ...
        this.responses.map((r) => r.response === 'red'),
        this.polarity,
        0.4
      )
    ...
    reset() {
      ...
      this.currentHue = Math.random() > 0.5 ? 240 : 360
      this.rounds = 0
      this.finalHue = 0
